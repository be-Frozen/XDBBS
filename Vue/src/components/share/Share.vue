    ...
    >
        ...
        <div class="message-panel">
            <div class="tab-list">
                <el-tabs :model-value="activeTabName" @tab-change="tabChange">
                    <el-tab-pane name="reply">
                        <template #label>
                            <div class="tab-item">
                                <span>回复我的</span>
                                <span class="count-tag" v-if="true">{{
                                        100 > 99 ? "99+" : 0
                                    }}</span>
                            </div>
                        </template>
                    ...
                    <el-tab-pane name="likePost">
                        <template #label>
                            <div class="tab-item">
                                <span>赞了我的文章</span>
                                <span class="count-tag" v-if="true">99+</span>
                            </div>
                        </template>
                    </el-tab-pane>
                ...
                <router-link :to="`/userhome/user/information`" class="a-link go-ucenter"
                >&lt;&lt;个人中心</router-link
                >
            </div>
...
<!--                    <img-->
<!--                        decoding="async"-->
<!--                        :src="'/'+'album/照片/'+this.$store.state.userName+'/'+this.nowalbum+'/'+i['p_address']"-->
...
<!--                <img-->
<!--                    decoding="async"-->
<!--                    :src="'/'+'album/照片/'+this.$store.state.userName+'/'+i"-->
<!--                    style="pointer-events: none; width: auto; height: auto; max-width: 100%; max-height: 100%;"-->
<!--                >-->
<!--            </div>-->
<!--                </div>-->
...
<!--                <img class="profile" :src="'/'+'album/头像/'+i['head']"/>-->
<!--                <div>-->
...
<!--                        <img decoding="async" :src="'/'+'album/照片/'+j"-->
<!--                            style="pointer-events: none; width: auto; height: auto; max-width: 100%; max-height: 100%;">-->
<!--                    </div>-->
<!--                </el-space>-->
...
<!--                            <img ref="imgDiv" class="myshareimg" @mousewheel="rollImg" @mousedown="move" decoding="async" :src="'/'+'album/照片/'+this.nowphoto">-->
<!--                        </div>-->
...
<!--                        <img style="position:absolute;float:left; left: 0; width: 40px;height: 40px;border-radius: 50%;" :src="'/'+'album/头像/'+j['head']"/>-->
...
export default {
    ...
    methods : {
        deleteshare(i){
            ...
            userxmlhttp.open("GET", '/' + phpaddr + "?sid="+arr[i]['id'], true);
        ...
        deletecomment(i,j){
            ...
            console.log(i);
            console.log(j);
            userxmlhttp.open("GET", '/' + phpaddr + '?id='+arr[i]['comment'][j]['id']+"&sid="+arr[i]['id']+"&commentnum="+arr[i]['commentnum'], true);
        ...
        like(index){    //点赞
            ...
            sharexmlhttp.open("GET", '/' + phpaddr +"?user="+Storage.state.userName+"&sid="+arr[index]['id']+"&like="+arr[index]['like']+"&likenum="+arr[index]['likenum'], true);
        ...
        ok(){
            ...
            userxmlhttp.open("GET", '/' + phpaddr + '?user='+Storage.state.userName+'&words='+this.sharewords, false);
            ...
            for (var i = 0; i < this.choosepicarr.length; i++) {
                ...
                photoxmlhttp[i].open("GET", '/' + phpaddr + '?user='+Storage.state.userName+'&album='+tempalbum+'&address='+tempaddress+'&sid='+userxmlhttp.id, true);
        ...
        uploadcomment(index){
            ...
            sharexmlhttp.open("GET", '/' + phpaddr +"?user="+Storage.state.userName+"&sid="+arr[index]['id']+"&words="+arr[index]['content']+"&commentnum="+arr[index]['commentnum'], true);
